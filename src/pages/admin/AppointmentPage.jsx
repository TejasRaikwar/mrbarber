    ...
    return (
        <div className="flex items-center gap-2.5 bg-card border border-border rounded-xl px-4 py-2.5">
            ...
            <div className="text-right">
                <p className="text-foreground font-semibold text-sm leading-tight tabular-nums">
                ...
                <div className="text-foreground">
                    <Calendar mode="single" selected={selected} autoFocus
                        ...
                        classNames={{
                            ...
                            outside: "text-muted-foreground/60", disabled: "opacity-25",
...
const TimePicker = ({ value, onChange }) => {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-muted border border-border rounded-lg p-0.5 ml-1">
                {["AM", "PM"].map((opt) => (
                    <button key={opt} type="button" onClick={() => setAmPm(opt)}
                        className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${ampm === opt ? "bg-(--brand) text-(--brand-foreground)" : "text-muted-foreground hover:text-foreground"}`}>
                        {opt}
                    </button>
                ))}
            </div>
...
const ApptCard = ({ appt, serviceOptions, onUpdated, onDelete }) => (
    <div className="bg-card border border-border rounded-2xl px-6 py-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-(--brand)/10 flex items-center justify-center shrink-0">
                <Scissors className="w-4 h-4 text-(--brand)" />
            </div>
            <div className="min-w-0">
                <p className="text-foreground font-semibold leading-tight">{appt.clientName}</p>
        ...
        <div className="hidden md:flex items-center gap-6 text-sm text-muted-foreground shrink-0">
            <span className="text-foreground/80">{appt.service}</span>
            ...
            <span className={`flex items-center gap-1 font-medium ${appt.amount != null ? "text-(--brand)" : "text-muted-foreground/70 italic"}`}>
                {appt.amount != null
        ...
        <>
                            ...
                            {FILTERS.map((f) => (
                                <button key={f.key} onClick={() => setFilter(f.key)}
                                    className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                                        filter === f.key ? "bg-(--brand) text-(--brand-foreground)" : "text-muted-foreground hover:text-foreground"
                                    }`}>
                                    {f.label}
                                </button>
                            ))}
            ...
            {groups.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h2 className="text-foreground font-semibold text-lg mb-2">No appointments</h2>
...
const inputCls = "w-full bg-muted border border-border focus:border-(--brand)/50 rounded-lg px-3 py-2.5 outline-none text-foreground text-sm transition-colors"
